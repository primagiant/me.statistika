...
@section('content')
            ...
            <div class="relative flex flex-col min-w-0 break-words bg-white w-full mb-6 shadow-lg rounded">
                    ...
                    <div class="flex flex-wrap items-center">
                        <div class="relative w-full px-4 max-w-full flex-grow flex-1">
                            <h3 class="font-semibold text-base text-gray-700">
                                Datalist Produk Moment
                            </h3>
                        </div>
                        <div class="relative w-full px-4 max-w-full flex-grow flex-1 text-right">
                            <form class="inline" action="{{ route('export.moment') }}" method="POST">
                                <button
                            ...
                            <form onsubmit="return confirm('Apakah anda yakin menghapus semua data?');"
                                class="inline" action="{{ route('truncate.moment') }}" method="POST">
                                <button
                ...
                <div class="block w-full overflow-x-auto">
                    ...
                    <table class="items-center w-full bg-transparent border-collapse">
                        ...
                        <tbody>
                            @foreach ($moment as $index => $item)
                                <tr>
                                    <th
                                        class="border-t-0 px-6 align-middle border-l-0 border-r-0 text-xs whitespace-nowrap p-4 text-center">
                                        {{ $moment->firstItem() + $index }}
                                    </th>
                                    <td
                                        class="border-t-0 px-6 align-middle border-l-0 border-r-0 text-xs whitespace-nowrap p-4 text-center">
                                        {{ $item->x }}
    ...
    <div class="hidden fixed z-10 inset-0 overflow-y-auto" id="importModal" aria-labelledby="modal-title" role="dialog"
        ...
        <div class="flex items-end justify-center min-h-screen pt-4 px-4 pb-20 text-center sm:block sm:p-0">
            ...
            <form action="{{ route('import.moment') }}" method="POST" enctype="multipart/form-data"
